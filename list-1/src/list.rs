...
impl<T> LinkedList<T>
{
    ...
    pub fn push_back(&mut self, elem: T)
    {
        // SAFETY: it's a linked-list, what do you want?
        unsafe {
            let new = NonNull::new_unchecked(Box::into_raw(Box::new(Node { back: None,
                                                                           front: None,
                                                                           elem })));
    ...
    pub fn back(&self) -> Option<&T> { unsafe { self.back.map(|node| &(*node.as_ptr()).elem) } }
...
impl<T> IntoIterator for LinkedList<T>
{
    ...
    fn into_iter(self) -> Self::IntoIter { IntoIter { list: self } }
}
